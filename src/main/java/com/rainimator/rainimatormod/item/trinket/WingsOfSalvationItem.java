...
public class WingsOfSalvationItem extends ItemBase implements Trinket, FabricElytraItem {
    ...
    public void tick(ItemStack stack, SlotReference slot, LivingEntity entity) {
        if (!entity.getWorld().isClient() && entity instanceof ServerPlayerEntity player) {
            if (ModConfig.getInstance().enableWingsCreativeFly && !player.isCreative())
                if (player.getAbilities().flying && !ManaComponent.tryUse(player, ManaConfig.getInstance().wings_of_salvation_second / 20))
    ...
    public void keyPress(PlayerEntity player, ItemStack stack) {
        if (player.isFallFlying())
            if (this.lastBoostTick <= 0 && ManaComponent.tryUse(player, ManaConfig.getInstance().wings_of_salvation_boost))
                this.speedUp(player);
    }
